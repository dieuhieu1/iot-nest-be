...
#define PIN_LED_D0 16   // D0
#define PIN_LED_D1 5    // D1
...
{
  int pin = -1;
  if (strcmp(deviceCode, DEVICE_LED_TEMP) == 0)
    pin = PIN_LED_TEMP;
  else if (strcmp(deviceCode, DEVICE_LED_HUM) == 0)
    pin = PIN_LED_HUM;
...
{
  if (!mqtt.connected())
    mqttReconnect(); // reconnect triggers init request automatically
  mqtt.loop();
  ...
  if (now - lastSensorPublish >= 2000)
  {
    ...
    // ── SERIAL LOG ────────────────────────────────────────────
    String lightLevel = lux < 256 ? "Very Dark" : lux < 512 ? "Dark"
                                              : lux < 768   ? "Bright"
                                                            : "Very Bright";
